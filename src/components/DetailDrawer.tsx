...
import { X, CheckCircle2, XCircle, Users, AlertTriangle, Layers, TrendingUp } from 'lucide-react';
...
export const DetailDrawer: React.FC<Props> = ({ result, onClose }) => {
            ...
            <div style={{ padding: '24px', borderBottom: '1px solid var(--border-color)', flexShrink: 0, background: r.indexing.isIndexed ? 'rgba(34,197,94,0.03)' : 'rgba(239,68,68,0.03)' }}>
              ...
              <div style={{ display: 'flex', gap: '24px' }}>
                ...
                <div style={{ flex: 1, background: 'var(--bg-app)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ fontSize: '0.9rem', fontWeight: 700, color: getScoreColor(r.score) }}>{r.score}/100</span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{r.score >= 90 ? 'Excelente' : 'Melhorar'}</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div style={{ padding: '20px 24px' }}>
                    {psSection === 'overview' && (
                      <div>
                        <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap', justifyContent: 'space-around' }}>
                          <CircleProgress score={r.pageSpeed.scores.performance} label="Desempenho" />
                          <CircleProgress score={r.pageSpeed.scores.accessibility} label="Acessibilidade" />
                          <CircleProgress score={r.pageSpeed.scores.bestPractices} label="Boas Práticas" />
                          <CircleProgress score={r.pageSpeed.scores.seo} label="SEO" />
                        </div>
                        ...
                        <div style={{ background: 'var(--bg-app)', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                            {[
                              { label: 'LCP', m: r.pageSpeed.lcp },
                              { label: 'CLS', m: r.pageSpeed.cls },
                              { label: 'INP', m: r.pageSpeed.inp },
                            ].map(({ label, m }) => (
                        ...
                        {r.pageSpeed.opportunities.length > 0 && (
                          <div>
                            ...
                            {r.pageSpeed.opportunities.slice(0, 3).map(op => (
                    ...
                    {psSection === 'metrics' && (
                      <div>
                        ...
                        <MetricRow label="First Contentful Paint (FCP)" metric={r.pageSpeed.fcp} description="Quando o primeiro texto ou imagem é exibido. Bom: ≤ 1.8s" />
                        <MetricRow label="Largest Contentful Paint (LCP)" metric={r.pageSpeed.lcp} description="Quando o maior elemento visível é renderizado. Bom: ≤ 2.5s" />
                        <MetricRow label="Total Blocking Time (TBT)" metric={r.pageSpeed.tbt} description="Tempo total de bloqueio da thread principal. Bom: ≤ 200ms" />
                        <MetricRow label="Cumulative Layout Shift (CLS)" metric={r.pageSpeed.cls} description="Mede a instabilidade visual (elementos pulando na tela). Bom: ≤ 0.1" />
                        <MetricRow label="Speed Index" metric={r.pageSpeed.speedIndex} description="Com que rapidez o conteúdo é exibido. Bom: ≤ 3.4s" />
                        <MetricRow label="Interaction to Next Paint (INP)" metric={r.pageSpeed.inp} description="Responsividade a interações do usuário. Bom: ≤ 200ms" />
                        <MetricRow label="Time to First Byte (TTFB)" metric={r.pageSpeed.ttfb} description="Tempo até o servidor enviar o primeiro byte. Bom: ≤ 800ms" />
                      </div>
                    )}
                      ...
                      <div>
                        {r.pageSpeed.opportunities.length === 0 ? (
                          ...
                          <>
                            ...
                            {r.pageSpeed.opportunities.map(op => (
                              <OpportunityRow key={op.id} op={op} />
                            ))}
                          </>
                        )}
                        {r.pageSpeed.blockers.length > 0 && (
                      ...
                      <div>
                        <div style={{ background: 'var(--bg-app)', borderRadius: '12px', padding: '16px', marginBottom: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                          <div>
                            ...
                            <div style={{ fontSize: '1.8rem', fontWeight: 800 }}>{r.pageSpeed.resources.totalRequests}</div>
                          </div>
                          <div>
                            ...
                            <div style={{ fontSize: '1.8rem', fontWeight: 800 }}>{r.pageSpeed.resources.totalSize}</div>
                          </div>
                        </div>
                        ...
                        <ResourceBar label="JavaScript" value={r.pageSpeed.resources.js} total={r.pageSpeed.resources.totalSize} color="#f59e0b" />
                        <ResourceBar label="Imagens" value={r.pageSpeed.resources.images} total={r.pageSpeed.resources.totalSize} color="#3b82f6" />
                        <ResourceBar label="CSS" value={r.pageSpeed.resources.css} total={r.pageSpeed.resources.totalSize} color="#8b5cf6" />
                        <ResourceBar label="Fontes" value={r.pageSpeed.resources.fonts} total={r.pageSpeed.resources.totalSize} color="#22c55e" />
                        <ResourceBar label="Outros" value={r.pageSpeed.resources.other} total={r.pageSpeed.resources.totalSize} color="#71717a" />
                      </div>
                    )}
                  </div>
                </>
